...
scoreboard objectives add spmob.config.semi_rare_prob dummy
...
data modify storage spmob:config nbt_data_main set value {CustomName:'お高い牛太郎'}

# レアモブ設定（配列形式で拡張性を確保）
# rare_mobs[0]=レアモブ、rare_mobs[1]=準レアモブ
# NBT未設定時はnbt_dataキーを含めない
data modify storage spmob:config rare_mobs set value [{mob_type:"minecraft:mooshroom",nbt_data:{Type:"brown",CustomName:'ムーシュ！'},probability:5},{mob_type:"minecraft:mooshroom",nbt_data:{CustomName:'ムーシュ！'},probability:10}]
